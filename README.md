# 📦 Sistema Automatizado de Limpeza, Análise e Monitorização de Inventário
...
Solução modular e automatizada de **Engenharia e Análise de Dados** desenvolvida em **Python** e **Pandas**, com suporte a **Matplotlib** e **Seaborn** para visualização executiva e **Git** para versionamento. O sistema realiza data profiling, higienização, mapeamento para schema canônico, cálculo de indicadores financeiros e deteção de roturas de inventário a partir de fontes de múltiplos fornecedores.

---

## 🎯 Problema de Negócio

Em ambientes de retalho e comércio eletrónico com múltiplos parceiros de abastecimento, a centralização dos dados de inventário enfrenta obstáculos críticos devido à **heterogeneidade e falta de governança na origem dos ficheiros**. 
...
* **Erros Ortográficos e Variações Textuais:** Nomes de produtos e departamentos escritos com gralhas e caixas variadas (`"Smartfone"`, `"smarthphone"`, `"FONE DE OUVIDO"`, `"audio / fone ouvido"`, `"Indussão"`, `"bluetoth"`).
...
* **Registos Nulos e Valores Inválidos:** Itens com preços ausentes ou zerados (`0,00`), campos de categoria vazios (`NaN`) e quantidades em inventário negativas (`-3`, `-10`, `-1`) ou registadas como `"SEM ESTOQUE"`.

**Objetivo:** Construir um pipeline resiliente e auditável em Python que converta ficheiros heterogéneos e inconsistentes numa base única, fidedigna (*Single Source of Truth*) e preparada para inteligência de negócio.
...
│
├── dados_brutos/                          # Ficheiros CSV brutos enviados pelos fornecedores
│   ├── fornecedor_a.csv
│   ├── fornecedor_b.csv
│   └── fornecedor_c.csv
│
...
├── analise_negocio.py                     # Cálculo de KPIs financeiros e deteção de roturas
...
1. **Padronização para Schema Canônico:**
   * Mapeamento explícito das colunas heterogéneas para o padrão unificado: `id_produto`, `produto`, `categoria`, `preco`, `estoque`, `fornecedor`.
...
2. **Coerção Numérica e Limpeza de Preços:**
   ...
   * **Auditoria de Descarte:** Registos com preços nulos ou `<= 0` (como itens em revisão a `0,00`) foram removidos com log detalhado, uma vez que preços financeiros não devem ser inventados sem regras de negócio acordadas.
...
3. **Tratamento de Estoques e Valores Negativos:**
   ...
   * Coerção numérica e retificação de quantidades negativas (`-3`, `-10`, `-1`) para `0` através de `.clip(lower=0)`, saneando inconsistências físicas de inventário.

4. **Normalização de Categorias com Expressões Regulares (Dedução Inteligente):**
   * Normalização de caixas e caracteres para um conjunto fechado de **3 categorias canónicas**: `Smartphones`, `Fones de Ouvido` e `Carregadores`.
...
| **Volume Total Físico em Inventário** | 553 unidades |
| **Preço Médio Global** | R$ 1.482,00 |
| **Capital Total Retido em Inventário** | **R$ 264.925,69** |
| **Itens em Rotura de Inventário (Estoque Zerado)** | 8 SKUs |
...
### 3. Identificação de Roturas de Inventário (Alerta de Reposição)
...
### 3. Alerta de Rotura de Inventário (Produtos Zerados por Ticket)
![Alerta de Rotura](graficos/alerta_ruptura_estoque.png)
...
### 4. Executar os Scripts na Ordem Adequada
...
# 4. Calcular métricas financeiras de negócio, alertas de rotura e exportar KPIs
